...
def field_average(ind, x, sumweights):
    return np.bincount(ind, weights=x.flatten(), minlength=len(sumweights)+1)[1:-1] / sumweights

def angular_average(x, kgrid, nbins):
    ind, bins, sumweights = get_binweights(kgrid, nbins)
    
    avg = field_average(ind, x, sumweights)
    
    return avg, bins

def power_spectrum(x1, x2=None, BoxSize=1.0):
    ft1, modes, grid = ft.fft(x1, BoxSize=BoxSize, modes=True, grid=True)
    
    if x2 is None:
        ft2 = ft1
    else: 
        ft2 = ft.fft(x2, BoxSize=BoxSize)
    N = x1.shape[0]
    pk = np.real(ft1 * np.conj(ft2))*(BoxSize**6/N**2)
    nbins = int(x1.shape[0] ** (2 / 2) / 2.1)
    P, kbins = angular_average(pk, grid, nbins)
